...
export const userService = {
  ...
  // Actions
  verifyEntity: async (id: number, type: 'clinic' | 'veterinary-clinic' | 'doctor' | 'paraprofessional') => {
    ...
    const response = await apiClient.patch(`/v3/admin/${endpoint}/${id}/${suffix}`);
    return response.data;
  },

  toggleStatus: async (id: number, type: 'store' | 'case' | 'clinic' | 'farm' | 'product' | 'pet', action: 'enable' | 'disable') => {

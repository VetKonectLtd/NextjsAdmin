...
export const blogService = {
    ...
    getAllBlogs: async (page = 1): Promise<BlogsResponse> => {
        const response = await apiClient.get<BlogsResponse>(`/v3/admin/get-all-blog?page=${page}`);
    ...
    deleteBlog: async (id: number): Promise<void> => {
        await apiClient.delete(`/v3/admin/delete-blog/${id}/comment`);
    },

    // Get deleted blogs
    getDeletedBlogs: async (): Promise<BlogsResponse> => {
        const response = await apiClient.get<BlogsResponse>('/v3/admin/blogs/deleted');
        return response.data;
    },
    ...
    // Flag a blog
    flagBlog: async (id: number, payload: FlagBlogPayload): Promise<void> => {
        await apiClient.post(`/v3/admin/blogs/${id}/flag`, payload);
    }
};
